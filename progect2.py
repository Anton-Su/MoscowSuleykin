...
class Player(Board):
    ...
    def move(self, carta, prisivnic=0):
        ...
        if len(self.pole[ryad]) < 10:
            # print(ryad)
            # to do: сделать проверку на карту - усилитель
    ...
    def ability(self, cart):
        ...
        if cart.ability == 'Trap':
            ...
            self.iicoloda = [i for i in self.iicoloda if i.ability != 'Prisivnic' \
                             or i.type != cart.type]
            f = len(self.iicoloda) - len(res) if \
            ...
            for i in range(len(res)):
                ...
                if not deistvie and f > 0:
                    f -= 1
                    self.iicoloda.insert(0, self.iicoloda[-1])
                    del self.iicoloda[-1]
                elif not deistvie and f == 0:
    ...
    while len(desk.coloda) > 0 or len(desk.iicoloda) > 0:
        ...
        for i in range(6):
            for ii in range(len(desk.pole[i])):
                if i < 3:
                    second += desk.pole[i][ii].cost
                else:
                    first += desk.pole[i][ii].cost
        # print(first, second)
